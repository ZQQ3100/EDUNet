...
class TrainDataset(DatasetBase):
    ...
    def load_event(self):
        ...
        for subroot in sorted(os.listdir(self.root)):
            ...
            eventlist.sort(key=lambda x: float(x[:-4]))
    ...
    def __getitem__(self, index):
        ...
        section_event_timestamp = cv_utils.read_npz_revd(event_path, 't')
        section_event_polarity = cv_utils.read_npz_revd(event_path, 'p')
        section_event_x = cv_utils.read_npz_revd(event_path, 'x')  # x,y exchange
        section_event_y = cv_utils.read_npz_revd(event_path, 'y')  # x-->[1,m]  change  x-->[0,m-1]
        start_timestamp = section_event_timestamp[0]
        end_timestamp = section_event_timestamp[-1]
        ...
        events_lst = event2frame.split_events_by_time(section_event, split_num=32, start_ts=start_timestamp, end_ts=end_timestamp)
        ...
        for e_idx, events_split in enumerate(events_lst): # events_split(N, 4)
            ...
            event_img = event2frame.event_to_cnt_img(events_split, height=blur.shape[1], width=blur.shape[2])
            event_img_lst.append(event_img) # split_num*2个元素的列表，每个元素都是形状为 (1, height, width) 的三维数组

        event_img_bins = np.concatenate(event_img_lst) # 合并之后变成形状为(split_num*2(64), height(720), width(1280)) 的三维数组
