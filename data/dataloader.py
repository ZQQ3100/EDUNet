...
class CustomDatasetDataLoader:
    def __init__(self, opt, is_for_train):
        ...
        self._num_threads = opt.n_threads
    ...
    def _create_dataset(self):
        ...
        if self._is_for_train:
            self._dataloader = torch.utils.data.DataLoader(
                ...
                shuffle=True,
                num_workers=2,
                # num_workers=int(self._num_threads),
                drop_last=True
            )
        else:
            self._dataloader = torch.utils.data.DataLoader(
    ...
    def __len__(self):
        return len(self._dataset)# 注意这里是self._dataset，不是self._dataloader，无论batch_size设成几都是训练数据的实际量数