...
import os
from code.data import DatasetBase
...
class Dataset(DatasetBase):
    ...
    def load_blur(self):
        ...
        for subroot in sorted(os.listdir(self.root_blur)):
            imgroot = os.path.join(self.root_blur, subroot, 'images')
    ...
    def load_event(self):
        ...
        for subroot in sorted(os.listdir(self.root_blur)):
            eventroot = os.path.join(self.root_blur, subroot, 'eventstream_mat')
    ...
    def __getitem__(self, index):
        ...
        blur_path = os.path.join(self.root_blur, dataname, 'images', blur_paths[img_idx])
        blur = cv_utils.read_cv2_img(blur_path, input_nc=1)

        # event images
        event_paths = self.event_paths.get(dataname)
        event_path = os.path.join(self.root_blur, dataname, 'eventstream_mat', event_paths[img_idx])
        ...
        section_event_x = cv_utils.read_mat_gopro(event_path, 'section_event_x') # x-->[0,m-1]
        section_event_y = cv_utils.read_mat_gopro(event_path, 'section_event_y') # y-->[0,n-1]
